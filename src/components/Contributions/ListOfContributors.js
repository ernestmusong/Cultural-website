...
const ListOfContributors=() => {
  ...
  const { contributions } = useSelector((store) => store.contributions);
        let bamenda = contributions.filter(contributor => contributor.branch === "bamenda");
        let bAmts = bamenda.map(person => person.amount)
        let bamendaTotal = bAmts.reduce((a, b) => {
          return a + b
        }, 0)
         

        let buea = contributions.filter(contributor => contributor.branch === "buea");
        let buAmts = buea.map(person => person.amount)
        let bueaTotal = buAmts.reduce((a, b) => {
          return a + b
        }, 0)
         

        let muea = contributions.filter(contributor => contributor.branch === "muea");
        let muAmts = muea.map(person => person.amount)
        let mueaTotal = muAmts.reduce((a, b) => {
          return a + b
        }, 0)
         
          
         let mutengene = contributions.filter(contributor => contributor.branch === "mutengene");
         let mutengAmts = mutengene.map(person => person.amount)
         let mutengTotal = mutengAmts.reduce((a, b) => {
           return a + b
         }, 0)
          

         let limbe = contributions.filter(contributor => contributor.branch === "limbe");
         let limbeAmts = limbe.map(person => person.amount)
         let limbeTotal = limbeAmts.reduce((a, b) => {
           return a + b
         }, 0)
          

         let yaounde = contributions.filter(contributor => contributor.branch === "yaounde");
         let yaoundeAmts = yaounde.map(person => person.amount)
         let yaoundeTotal = yaoundeAmts.reduce((a, b) => {
           return a + b
         }, 0)
          

         let douala = contributions.filter(contributor => contributor.branch === "douala");
         let doualaAmts = douala.map(person => person.amount)
         let doualaTotal = doualaAmts.reduce((a, b) => {
           return a + b
         }, 0)
          

         let diaspora = contributions.filter(contributor => contributor.branch === "diaspora");
         let diasporaAmts = diaspora.map(person => person.amount)
         let diasporaTotal = diasporaAmts.reduce((a, b) => {
           return a + b
         }, 0)
          

         let beyoin = contributions.filter(contributor => contributor.branch === "beyoin");
         let beyoinAmts = beyoin.map(person => person.amount)
         let beyoinTotal = beyoinAmts.reduce((a, b) => {
           return a + b
         }, 0)
      ...
      if(contributions.length === 0) return <CommingSoon title="there are no contributions at the moment!" />;
  return (
    <>
